...
for tag in arm64_tahoe x86_64_linux
do
...
rejected_names=(
  ...
  "other--${version}.arm64_tahoe.bottle.tar.gz"
)
for rejected in "${rejected_names[@]}"
do
  bottle="${fixture_dir}/${rejected}"
  printf 'bottle\n' >"${bottle}"
  if "${verifier}" "${bottle}" "${version}" 2>/dev/null
  then
...
: >"${empty_bottle}"
if "${verifier}" "${empty_bottle}" "${version}" 2>/dev/null
then
