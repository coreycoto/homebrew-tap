...
require_text "${publisher}" '[.[][] | select(.tag_name == $tag)] as $matches'
require_text "${publisher}" 'repos/${GITHUB_REPOSITORY}/releases/tags/${release_tag}'
require_text "${publisher}" '.immutable == true'
require_text "${publisher}" 'Bottle release ${release_tag} did not become immutable after publication.'
